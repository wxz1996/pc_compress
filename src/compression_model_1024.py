...
def input_fn(features, batch_size, preprocess_threads,  repeat=True, prefetch_size=1, is_train = True):
    with tf.device('/cpu:0'):
        ...
        dataset = tf.data.Dataset.from_generator(lambda: iter(features), tf.float32, tf.TensorShape([1024, 3]))
        # create the iterator
        if repeat:
...
def pc_decoder(y_tilde, batch_size, is_training, bn_decay):
    ...
    net = tf_util.conv2d_transpose(net, 256, kernel_size=[3, 2], stride=[2, 1], padding='VALID', scope='upconv2',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    net = tf_util.conv2d_transpose(net, 256, kernel_size=[3, 3], stride=[3, 2], padding='VALID', scope='upconv3',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    net = tf_util.conv2d_transpose(net, 128, kernel_size=[4, 4], stride=[2, 2], padding='VALID', scope='upconv4',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    ...
    net = tf_util.fully_connected(net, 512 * 3, activation_fn=None, scope='fc3')
