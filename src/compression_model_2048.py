...
def input_fn(features, batch_size, preprocess_threads,  repeat=True, prefetch_size=1, is_train = True):
    with tf.device('/cpu:0'):
        ...
        dataset = tf.data.Dataset.from_generator(lambda: iter(features), tf.float32, tf.TensorShape([2048, 3]))
        # create the iterator
        if repeat:
...
def model_fn(features, labels, mode, params):
    ...
    if params.decompress:
        ...
        y_tilde = entropy_bottleneck.decompress(features, [1024], channels=1024)  # B*N
...
def pc_encoder(point_cloud, nasmples, is_training, bn_decay=None):
    ...
    net = tf_util.fully_connected(net, 1024, bn=True, is_training=is_training, scope='fc01', bn_decay=bn_decay)
    net = tf_util.fully_connected(net, 1024, bn=True, is_training=is_training, scope='fc02', bn_decay=bn_decay)
    net = tf.reshape(net, [batch_size, -1])
    return net

def pc_decoder(y_tilde, batch_size, is_training, bn_decay):
    # UPCONV Decoder
    net = tf.reshape(y_tilde, [batch_size, 1, 1, 1024])
    ...
    net = tf_util.conv2d_transpose(net, 256, kernel_size=[3, 3], stride=[1, 1], padding='VALID', scope='upconv2',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    net = tf_util.conv2d_transpose(net, 256, kernel_size=[4, 4], stride=[2, 2], padding='VALID', scope='upconv3',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    net = tf_util.conv2d_transpose(net, 128, kernel_size=[5, 5], stride=[3, 3], padding='VALID', scope='upconv4',
                                   bn=True, bn_decay=bn_decay, is_training=is_training)
    ...
    # FC Decoder
    net = tf_util.fully_connected(y_tilde, 1024, bn=True, is_training=is_training, scope='fc1', bn_decay=bn_decay)
    net = tf_util.fully_connected(net, 2048, bn=True, is_training=is_training, scope='fc2', bn_decay=bn_decay)
